...
void outputMessage(const float x1, const float y1, const float x2, const float y2, const float x3, const float y3, const float area, const int possible) {
  if (possible) {
...
int main(const int argc, const char* const argv[]) {
  ...
  if (argc != EXPECTED_ARG_COUNT){
    printf("Expected 6 integer inputs; received\n");
    return 0;
  }
  ...
  int possible = area > 0.0;

  /* Output area */
  outputMessage(x1,y1,x2,y2,x3,y3,area,possible);
  return 0;
