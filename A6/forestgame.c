...
static bool check_win(const struct forest *frst) {
  assert(frst);
  int countInPlayers = 0;
  int countInGhosts = 0;
  int stepped = 0;
  
  for (int i = 0; i < (frst->height * frst->width); i++) {

    if (frst->player_map[i] != MARKED && frst->player_map[i] != UNREVEALED){
      stepped++;
    }
    if (frst->player_map[i] == MARKED && frst->ghost_map[i] == ghost_presence) {
      countInPlayers++;
      // printf("P%d S%d", countInPlayers, stepped);
    }
    if (frst->ghost_map[i] == ghost_presence) {
      countInGhosts++;
      // printf("G%d %d\n", countInGhosts, i);
    }
    // printf("P%d S%d G%d %d\n", countInPlayers, stepped, countInGhosts, i); 
     
  }

  if (stepped == ((frst->height * frst->width)-countInGhosts)){
    return true;
  }
  // printf("NOT IT");

  return countInPlayers == countInGhosts;
}
...
int forest_reveal(struct forest *frst, int x, int y) {
  assert(frst); int pos = transfer_2D_into_1D_pos(frst, x, y);
  ...
  if (frst->ghost_map[pos] == ghost_presence){
    frst->player_map[pos] = ghost_presence;
  ...
  if (frst->ghost_map[pos] == ghost_absence){
